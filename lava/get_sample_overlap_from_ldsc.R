...
## adapted from LAVA TUTORIAL (Josefin Werme, CTG Lab, VU Amsterdam)
## https://github.com/josefin-werme/LAVA/blob/main/vignettes/sample_overlap.md
#------------------------------------
# command line arguments, specifying phenotypes to include
args = commandArgs(trailingOnly=TRUE)
sumstats2read<-args[1]
# get name of project, to use in output
prefix=gsub("_sumstats2read.txt","",sumstats2read)
#------------------------------------
...
phenos2check<-read.table(paste0(lava_dir,sumstats2read))$V1
#------------------------------------
...
input_files2<-list.files(paste0(ldsc_dir,"../output/"),pattern="table")
input_files2<-input_files2[grep("rg",input_files2)]
input_files2<-paste0(ldsc_dir,"../output/",input_files2)
...
write.table(mat, paste0(lava_dir,prefix,"_","sample.overlap.txt"), quote=F)   # save
